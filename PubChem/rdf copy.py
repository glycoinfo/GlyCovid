...
def check_genedir(d_today):
    dirname = "data/"+d_today+"/gene"
    files = os.listdir(dirname)
    folders = [f for f in files if os.path.isdir(os.path.join(dirname, f))]
    genelist = pd.read_csv(
        './datalist_csv/PubChem_gene_text_covid-19_summary_'+d_today+'.csv')
    genelist = list(genelist['geneid'])

    diff = copy(genelist)
    for i in range(len(diff)-1, -1, -1):
        for j in range(len(folders)):
            if diff[i] == int(folders[j]):
                diff.pop(i)
                break

    print(diff)
    print('gene: ', len(genelist), '\tdirs: ',
          len(folders), '\tdiff: ', len(diff))
    return diff
...
def get_new_gene_list(d_totay, path):
    ...
    # print(len(list(now_genelist['csv']['geneid'])))
    # print(list(now_genelist['csv']['geneid'])[0:10])
    # print(len(list(current_genelist['csv']['geneid'])))
    # print(list(current_genelist['csv']['geneid'])[0:10])
    gene_list = list(now_genelist['csv']['geneid'])
...
def scrape(d_today, path, gene_list):
    ...
    for i in range(len(gene_list)):
        ...
        try:
            ...
            ####################
            # elements = driver.find_elements_by_xpath(
                # "//a[@data-action='download-sction']/span[text()='CSV']")
        except:
            traceback.print_exc()
        finally:
            driver.quit()
...
def curate_csv_file(d_today):
    ...
    # /Users/Kouiti/local_file/glycovid/glycovid_PubChem/data/2021-07-28/dir/bioassay_s.csv
    for i in range(len(file_list)):
        with open(file_list[i], 'r') as f:
            ...
            # if 'chembldrug_s' not in file_list[i]:
...
if __name__ == "__main__":
    ...
    # 取りこぼしたデータをスクレイピング
    # failed_genelist = check_genedir()
    # scrape(d_today, path, failled_genelist)
